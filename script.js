...
function validateAndCleanCart() {
  ...
  const updatedCart = cart.filter((cartItem) => {
    // 1. Buscar el producto padre por marca y nombre
    const parentProduct = PRODUCTS.find(
      (p) => p.brand === cartItem.brand && p.name === cartItem.name
    );

    if (!parentProduct || parentProduct.outOfStock) {
      const title = cartItem.brand ? `${cartItem.brand} ${cartItem.name}` : cartItem.name;
      removedItems.push(`${title} (${cartItem.flavor})`);
      return false;
    }

    // 2. Buscar la variante específica de sabor
    const flavorObj = parentProduct.flavors.find(
      (f) => f.name === cartItem.flavor
    );

    // Si el sabor no existe o está marcado como sin stock
    if (!flavorObj || flavorObj.outOfStock) {
      const title = cartItem.brand ? `${cartItem.brand} ${cartItem.name}` : cartItem.name;
      removedItems.push(`${title} (${cartItem.flavor})`);
      return false;
    }
  ...
  // Si hubo modificaciones, guardamos y notificamos
  if (cart.length !== updatedCart.length) {
    saveCart(updatedCart);
    if (removedItems.length > 0 && typeof showToast === "function") {
      showToast(`Se quitaron por falta de stock: ${removedItems.join(", ")}`);
    }
